...
| Phase 9B | ✅ 已完成 | PR 前最终收尾：README 旧配置清理（移除 `spring.datasource.*` / `ai.deepseek.api-key` / `qwen.api-key` / `jwt.secret` / `app.redis.enabled` 等旧 property 表与 `application.yml` 直改示例，改为指向 `.env` / `docs/deployment-config.md` / `docs/manual-deployment-runbook.md`；JDK 17+→21；删除已落地的 `QwenDocumentParseService.java` / `LlamaParseService.java` 在 README 中的过时引用）、部署文档一致性校验（runbook 与 `application.yml` / `application-ai.yml` / `.env.example` / `package.json` / `pom.xml` 对齐 Java 21 / Spring Boot 3.2.3 / context-path `/api` / Vite 3000 / `VITE_API_BASE_URL`）、最终安全 grep（`sk-` / `Bearer ` / `Authorization` / `apiKey=` 等均落在允许区域：测试 fake key、脱敏工具、统一客户端、文档示例；前端 localStorage 不含 apiKey；旧 `callDeepSeek` / `QwenDocumentParseService` 已删；无 Docker / docker-compose / 自动部署 / GitHub Secrets / 真实 secret）、最终 `mvn test` + `npm run build`、PR 描述模板（`docs/pr-description-draft.md`）。未改业务逻辑 / DB 结构 / AI Provider 调用链 / quota / rate limit / Admin 权限逻辑 |
